...
extern "C"
{
	...
	__declspec(dllexport) int DoubleLimitedSearch(int* boardArr, int beamWeight, int limitWeight, int startRow, int startColumn, int* outRoute)
	{
		BOARD board = toBoard(boardArr);

		int score;
		auto route = doubleLimitedSearch(board, beamWeight, limitWeight, startRow, startColumn, score);
...
int main(void)
{
	...
	while (true)
	{
		evalComboTest();
		//auto board = inputBoard();
		////countReachTest(board);
		//int combo = evalProcess(board);
		//cout << combo << endl;
		//debug(board);
