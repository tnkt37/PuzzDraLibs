...
namespace tnkt37
{
	namespace PuzzDraSolver
	{
		...
		public:
			...
			//最初のルート
			Route(const int& initialRoute)
			{
				bit1 = (1 << 3) | initialRoute;
				bit2 = 1LL;
			}
			...
			{
				return bit1 == route.bit1 && bit2 == route.bit2;
			}

			bool operator<(const Route& route) const
			{
				if (bit2 != route.bit2) return bit2 < route.bit2;
				return bit1 < route.bit1;
			}
