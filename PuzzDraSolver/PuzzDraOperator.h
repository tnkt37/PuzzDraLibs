...
namespace tnkt37
{
	namespace PuzzDraSolver
	{
		...
		//移動後のドロップの位置を返す(0~2bit目でrow, 3~5bit目でcolumn)
		...
		inline void checkConnect(BOARD& board, UnionFind& comboTree, int pos, int adjacentPos)
		{
			...
			int size = comboTree.size(adjacentPos);
		...
		inline void comboColumn(BOARD& board, UnionFind& comboTree, int row, int column)
		{
			...
			checkConnect(board, comboTree, pos, pos - WIDTH);//一段上
			checkConnect(board, comboTree, pos, pos + 1 - WIDTH);//右上
			checkConnect(board, comboTree, pos, pos + 2 - WIDTH);//右上の右
		...
		inline int countReach(BOARD& board)
		{
			...
			for (int column = 0; column < WIDTH; column++)
			{
				for (int row = 0; row < HEIGHT; row++)
				{
					...
					if (row + 1 < HEIGHT && board[row + 1][column] == EMPTY) continue;//一個下も空きだったら処理しない
		...
		//評価や盤面の押し下げ等の一連の処理を行う
		//返り値は評価値
		inline int evalProcess(BOARD& board, int& out_combo)
		{
			int score = 0;
			out_combo = 0;
			while (true)
			{
				int combo = evalCombo(board);
				if (combo == 0) break;
				score += combo * 100;//リーチより価値を高くするための100倍

				//普通のリーチ(2個並んでるパターン)の数も加味したい

				score *= 1;//初回でコンボしたほうがいいみたいな評価にしたい

				out_combo += combo * 1000;//計測用に差を大きく
				bool pushed = pushDownDrops(board);
				if (!pushed) break;
			}
			int reaches = countReach(board);
			score += reaches;
			out_combo += reaches;
